 ...
 * Simple program to check the functionality of lock_acquire() and
 * lock_release()
 */
...
void thread2(void) {
    ...
    // This thread initializes the lock
    // The other thread waits for init to become TRUE
    if (!init) {
        lock_init(&l);
        init = TRUE;
    }
    ...
    for (i = 0; i < 100; i++) {
        ...
        if (i % 13 == 0) {
            // Yielding inside the critical section might trigger some errors
            // in bad implementations of the locks
            do_yield();
        }
...
void thread3(void) {
    ...
    for (i = 0; i < 100; i++) {
        ...
        // shared_var should be equal to tmp
        // If the synchronization primitives are not working correctly then
        // shared_var > tmp, because thread2 has increased shared_var
        shared_var = tmp + 1;
        // Done using shared_var
        lock_release(&l);
