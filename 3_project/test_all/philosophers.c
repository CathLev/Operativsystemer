...
enum {
    ...
    COL = 48          // Column where the result should be printed
};

static volatile bool_t eating[NUM_PHILOSOPHERS];
...
static void philosopher(int id) {
    ...
    while (TRUE) {
        status(id, "thinking");
        do_sleep(MIN_THINK_TIME + rand() % (MAX_THINK_TIME - MIN_THINK_TIME));
        status(id, "hungry  ");
        if (0 == id) {
